"""Task visibility affects layout, not the underlying plan or tool history."""
...
def test_preference_and_command(tmp_path, monkeypatch):
...
def test_reset_preserves_hidden_preference():
    activity = Activity(show_tasks=False)
    activity.reset()
