...
- `/links`: pick a URL from the active conversation branch (your prompts, tool
  arguments and captured results, or the assistant's replies, newest first).
  Tool links show the tool name; duplicate URLs appear once. Output omitted by
  truncation or stored only in a spill file is not searched. Open the selection
